...
public class TableServiceClient {
    ...
    public void updateCleaningStatus(Long tableId, String status, String staffEmail, String note) {
        try {
            webClient.put()
                    .uri("/api/tables/{id}/cleaning", tableId)
                    .bodyValue(Map.of("status", status, "staffEmail", staffEmail, "note", note))
                    .retrieve()
                    .bodyToMono(String.class)
                    .block();
        } catch (Exception e) {
            log.warn("Failed to update cleaning status of table {} to {}", tableId, status);
        }
    }
}
